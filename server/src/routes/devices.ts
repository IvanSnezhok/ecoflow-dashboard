...
router.get('/', async (_req: Request, res: Response) => {
  try {
          ...
          try {
            ...
            const parseExtraBattery = (prefix: string) => {
              ...
              if (soc === undefined || soc === null) {
                const remainCap = q[`${prefix}.remainCap`] as number | undefined
              ...
              const temp = (q[`${prefix}.temp`] as number) ??
                           (q[`${prefix}.minCellTemp`] as number) ?? 0

              // SOH - fallback to cycleSoh
              const soh = (q[`${prefix}.soh`] as number) ??
                          (q[`${prefix}.cycleSoh`] as number) ?? 100
            ...
            state = {
              ...
              solarInputWatts: (q['mppt.inWatts'] as number) || ((q['mppt.inAmp'] as number || 0) * (q['mppt.inVol'] as number || 0) / 1000) || 0,
              ...
              acOutEnabled: (q['inv.cfgAcEnabled'] as number) === 1 || (q['inv.acOutState'] as number) === 1,
              dcOutEnabled: (q['mppt.carState'] as number) === 1 || (q['pd.carState'] as number) === 1,
              ...
              extraBattery1: (() => {
                ...
                const cached = extraBatteryCache.get(apiDevice.sn)?.extraBattery1
              ...
              extraBattery2: (() => {
                ...
                // Return cached data if available (within 5 minutes)
                const cached = extraBatteryCache.get(apiDevice.sn)?.extraBattery2
                if (cached && Date.now() - cached.lastUpdated < 5 * 60 * 1000) {
                  const { lastUpdated, ...batteryData } = cached
                  return batteryData
                }
    ...
    insertLog(null, 'API_CALL', 'getDevices', null, JSON.stringify({ count: devices.length }), true, null)
    res.json(devices)
  } catch (error) {
...
router.get('/:sn', async (req: Request, res: Response) => {
  try {
    const { sn } = req.params
    const device = getDeviceBySn(sn) as { id: number; serial_number: string; device_type: string; name: string; online: number } | undefined
...
router.post('/:sn/ac-output', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, message: `AC output ${enabled ? 'enabled' : 'disabled'}` })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error'
...
router.post('/:sn/dc-output', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, message: `DC output ${enabled ? 'enabled' : 'disabled'}` })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error'
    const device = getDeviceBySn(req.params.sn) as { id: number } | undefined
    insertLog(
...
router.post('/:sn/charge-limit', async (req: Request, res: Response) => {
  try {
    ...
    if (maxSoc < 50 || maxSoc > 100 || minSoc < 0 || minSoc > 30) {
      res.status(400).json({ error: 'maxSoc must be 50-100, minSoc must be 0-30' })
      return
    }
    ...
    res.json({ success: true, message: `Charge limit set: ${minSoc}% - ${maxSoc}%` })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error'
    const device = getDeviceBySn(req.params.sn) as { id: number } | undefined
    insertLog(
...
function calculateTimeRange(period: ChartPeriod): { from: string; to: string; aggregation: AggregationType } {
  const now = new Date()
...
router.get('/:sn/history', async (req: Request, res: Response) => {
  try {
    const { sn } = req.params
    const { period = '24h' } = req.query as { period?: ChartPeriod }
    ...
    const { from, to, aggregation } = calculateTimeRange(period as ChartPeriod)
